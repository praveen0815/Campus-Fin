  ...
  return (
    <section className="space-y-8">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-600">Monitor and control every student booking in one place.</p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">Confirmed</p>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {bookings.map((booking) => (
                  <tr key={booking.id} className="transition-all hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-700">{booking.slot?.venues?.sports?.name ?? 'Unknown'}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{booking.slot?.venues?.name ?? 'Unknown'}</td>
